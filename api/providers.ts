...
export default async function handler(req: any, res: any) {
  ...
  try {
    const url = `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/${FIREBASE_DATABASE_ID}/documents/providers?pageSize=100&key=${FIREBASE_API_KEY}`;
    const response = await axios.get(url, { timeout: 6000 });
    const documents = response.data.documents || [];
    const providers = documents.map((doc: any) => {
    ...
    console.error("[VERCEL-API-PROVIDERS] Error:", err.message);
