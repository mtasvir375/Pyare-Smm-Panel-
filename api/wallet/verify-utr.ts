...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // 1. Check in bank_alerts or sms_forwarder_pool
    ...
    if (!alertDoc) {
      // Save to pending_user_utrs so when Telegram SMS arrives later, it auto-credits!
      try {
    ...
    // 5. Mark as used in both collections
    const updatedAlert = {
      ...alertDoc,
      isUsed: true,
      status: "claimed",
